/**
 * Skill 9: Narrative Constraint Generator（扩展版 = 基础版 + 三账号策略）
 *
 * 功能：
 ...
 *
 * 扩展版（原 narrative-constraint-generator-extended.js）与基础版（原 narrative-constraint-generator.js）
 * 已合并为单一文件。
 * pipeline 通过 generateConstraints() 调用此文件，等效于调用扩展版。
 */
...
const CONTENT_DEEPENING_RULES = {
  ...
  trigger_conditions: {
    ...
    to_multi_platform: [
      { signal: 'high_feedback', threshold: 'any_2_of_3' }
    ]
  },
...
/**
 * 生成扩展约束（含三账号策略）
 * 基础约束（generateConstraints）直接内联，不通过 require，
 * 避免覆盖后的循环依赖问题。
 */
function generateExtendedConstraints(strategyCard) {
  const base = generateConstraints(strategyCard);
...
// ===== 导出 =====
// pipeline 调用 generateConstraints，指向扩展版
module.exports = {
  generateConstraints: generateExtendedConstraints,  // pipeline 实际调用这个
