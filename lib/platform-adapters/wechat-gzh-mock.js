...
class WechatGzhMockAdapter extends PlatformAdapter {
  constructor() {
    ...
    this.avgCTR = 0.025;
    this.stdCTR = 0.015;
    this.contentForms = ['图文', '图文', '图文', '短视频'];
  }
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return makeArticlePerformance({
        ...
        metrics: {
          ...
          clicks,
          ctr,
          conversions: Math.round(clicks * (0.01 + Math.random() * 0.05)),
          cvr: 0.01 + Math.random() * 0.05,
          shares: Math.floor(clicks * 0.05),
          likes: Math.floor(clicks * 0.1),
          comments: Math.floor(clicks * 0.02),
          avg_read_time: 60 + Math.floor(Math.random() * 300)
        },
        metadata: {
          content_form: pickOne(this.contentForms),
          topic_keywords: []
        }
      });
    });
